...
class LoginPresenter(context: Context, private val mView: LoginContract.View) : BasePresenter(context, mView), LoginContract.Presenter, Response.Listener<BaseModel<Token>> {

    override fun login(user: User) {
        if(mView.isInternetWorking()){
            if(validateUser(user)){
                mView.showProgressDialog()
                UserApi().login(mSharedPreferenceManager.getString(SharedPreferenceManager.KEY_PUBLIC_ACCESS_TOKEN,"")
                        , user
